...
{
    public class Cs_Vendedor_Dados : Cs_Conexao
    {
        MySqlCommand cmd;
        ...
        public object Cadastrar(string nome, string sobrenome,char genero,string bi,DateTime nascimento,DateTime datAdmissao,string numCredencial,bool status,string provincia,string municipio, string bairro, string rua, string casa,string telefone,string email)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Clear();
                cmd.CommandText = "INSERT INTO `tbl_vendedor`(id_Pessoa, dataAdmissao, num_Credencial, status) VALUES (@id_Pessoa, @dataAdmissao, @num_Credencial, @status)";
        ...
        public DataTable CarregarTodos()
        {
            ...
            try
            {
                MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT *FROM vwVendedor", Conexao);
                Conectar();
                adapter.Fill(tabela);
            }
            catch (Exception ex)
